"""
Download Massive.com daily stock candlestick flat files (last 5 years)
and concatenate into a single CSV.

Requirements:
    pip install boto3 tqdm python-dotenv

Add a .env file in the project root (~/dev/hades/.env) with:
    MASSIVE_ACCESS_KEY=your_access_key
    MASSIVE_SECRET_KEY=your_secret_key

Get your keys from: https://massive.com/dashboard/keys

Output:
    ~/dev/hades/stockdata/stocks_daily.csv
"""
...
OUTPUT_FILE = OUTPUT_DIR / "stocks_daily.csv"

WORKERS = 50  # concurrent download threads
...
def download_file(file_info: dict, tmpdir: str) -> tuple[dict, Path | None]:
    """Download and decompress one file. Returns (file_info, csv_path | None)."""
    key = file_info["key"]
    suffix = ".csv.gz" if key.endswith(".gz") else ".csv"
    # Use a unique name per file so threads don't collide
...
def main():
    ...
    # Download all files in parallel, preserving date order for CSV output
    results: dict[str, Path | None] = {}  # date_str -> csv_path
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Write to CSV in sorted date order (sequential — no threading needed)
        header_written = False
        with open(OUTPUT_FILE, "w") as out_f:
            for file_info in files:  # already sorted by date
